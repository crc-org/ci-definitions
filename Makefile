...
.PHONY: crc-builder-oci-build crc-builder-oci-push crc-builder-tkn-create crc-builder-tkn-push
...
crc-builder-oci-build:
	...
	${CONTAINER_MANAGER} build --platform linux/arm64,linux/amd64 --manifest $(MANIFEST)-linux -f $(CONTEXT)/Containerfile.linux $(CONTEXT)
...
crc-builder-oci-push:
	${CONTAINER_MANAGER} manifest push $(MANIFEST)-linux
